...
def check_task_status(api_key, task_id, max_attempts=30, delay=10):
    """
...
def get_acceptance_report(api_key, date_from, date_to, supplier):
    ...
    try:
        ...
        logger.info(f"[{supplier}] Получено {len(df)} записей")
        return df
...
def get_acceptance_reports_by_all_ip(dict_api, start_date, end_date):
    ...
    Обрабатывает каждого клиента отдельно для оптимизации памяти.
    ...
    for api, supplier in dict_api.items():
        ...
        try:
            # Получаем данные для текущего поставщика
            df = get_acceptance_report(api_key=api, date_from=start_date, date_to=end_date, supplier=supplier)
            ...
            if df is not None and not df.empty:
                ...
                ensure_table_columns(engine, 'reports', 'acceptance', df)
                
                # Загружаем данные в PostgreSQL
                df.to_sql(name='acceptance', con=engine, schema='reports', if_exists='append', index=False)
                logger.info(f"[{supplier}] ✓ Загружено {len(df)} записей в БД")
                ...
                logger.warning(f"[{supplier}] Нет данных для загрузки")
    ...
    if success_count > 0:
        logger.info(f"✓ Среднее записей на поставщика: {total_records // success_count}")
    logger.info("=" * 80)


def delete_old_acceptance_records(engine):
    """
    Удаляет старые записи из таблицы reports.acceptance, оставляя только последнюю запись для каждого поставщика и даты.

    :param engine: Экземпляр SQLAlchemy Engine для подключения к базе данных.
    """
    query = """
    DELETE FROM reports.acceptance a
    WHERE a.update_time < (
        SELECT MAX(a2.update_time)
        FROM reports.acceptance a2
        WHERE a.supplier = a2.supplier
        AND a.shkcreatedate = a2.shkcreatedate
    );
    """
    ...
    try:
        with engine.begin() as connection:
            ...
            logger.info(f"✓ Удалено {deleted_rows} устаревших записей из таблицы acceptance")
    except Exception as e:

#!/usr/bin/env python3
"""
Скрипт для исправления несоответствий в данных платной приемки
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('acceptance_fix.log'),
...
def get_discrepancies_to_fix(date_from, date_to):
    ...
    Returns:
        pd.DataFrame: Данные о несоответствиях
    """
    try:
        ...
        WITH cte_acceptance AS (
            ...
            FROM (
                ...
                WHERE a."shkсreatedate" >= :date_from::date
...
def fix_detail_finance_reports(df_discrepancies, dry_run=True):
    ...
    Args:
        df_discrepancies (pd.DataFrame): Данные о несоответствиях
        dry_run (bool): Если True, только показывает что будет исправлено
    """
    try:
...
def recalculate_aggregated_report(date_from, date_to, dry_run=True):
    ...
    Args:
        date_from (str): Дата начала периода
        date_to (str): Дата окончания периода
        dry_run (bool): Если True, только показывает что будет пересчитано
    """
    try:
    ...
    try:
        ...
        if len(df_discrepancies) > 0:
            ...
            if dry_run:
                ...
                logger.info("   Пример: python run_acceptance_fix.py 2024-12-01 2024-12-31 --execute")
        else:
            logger.info("✅ Несоответствий для исправления не найдено")

...
def upload_orders_to_db(engine, df):
    ...
    table = meta.tables['reports.orders']
    ...
    do_update_stmt = insrt_stmnt.on_conflict_do_update(
        index_elements=["srid"],  # Уникальный индекс (конфликт по полям индекса)
        set_={  # Указываем, какие поля будут обновляться
            ...
            "iscancel": insrt_stmnt.excluded.iscancel,
            "canceldate": insrt_stmnt.excluded.canceldate,
...
for date in formatted_dates:
    for api_key, supplier_name in dict_api.items():
        ...
        if res is not None and len(res) > 0:  # Проверка, что результат не пустой
            ...
            # save_path = r"C:\Users\vivar\PycharmProjects\wb_baah\tasks\ANALITIKAVB-6\check_data"
            # os.makedirs(save_path, exist_ok=True)  # Создает папку, если её нет
            #
            # # Формируем имя файла
            # file_name = f"{date}_{supplier_name}.json"
